...
function ScoreRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded border border-zinc-800/60 bg-zinc-900/30 px-4 py-3">
      ...
      {onPlay && !isPerfect && (
        <button
          className="shrink-0 rounded border border-zinc-700 px-2 py-1 text-xs font-semibold text-zinc-300 hover:border-zinc-500 hover:text-zinc-100"
          type="button"
          onClick={() => onPlay(entry.levelNumber)}
        >
          Play
        </button>
      )}
  ...
  return (
    <main className="min-h-dvh bg-[#050505] text-zinc-100">
      <header className="flex items-center gap-3 border-b border-zinc-800/70 px-5 py-4">
        ...
        <h1 className="text-base font-semibold">High scores</h1>
      </header>
